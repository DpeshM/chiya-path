...
const ExpensesTab = ({ expenses, setExpenses, completedTransactions, handleSyncToGoogle }) => {
  ...
  const addExpense = async () => {
    ...
    await handleSyncToGoogle('expenses', newExpenses);
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4 mb-4 sm:mb-6">
        <div className="p-3 sm:p-4 bg-orange-50 border-2 border-orange-500 rounded-lg">
          ...
          <p className="text-xl sm:text-2xl font-bold text-orange-600">
            Rs.{expenses.reduce((sum, e) => sum + e.amount, 0).toFixed(2)}
          </p>
        </div>
        <div className="p-3 sm:p-4 bg-green-50 border-2 border-green-500 rounded-lg">
          ...
          <p className="text-xl sm:text-2xl font-bold text-green-600">
            Rs.{completedTransactions.reduce((sum, t) => sum + t.total, 0).toFixed(2)}
          </p>
        </div>
        <div className="p-3 sm:p-4 bg-blue-50 border-2 border-blue-500 rounded-lg">
          ...
          <p className="text-xl sm:text-2xl font-bold text-blue-600">
            Rs.{(completedTransactions.reduce((sum, t) => sum + t.total, 0) - expenses.reduce((sum, e) => sum + e.amount, 0)).toFixed(2)}
          </p>
        </div>
      </div>

      {expenses.length === 0 ? (
        <p className="text-gray-500 text-center py-8 sm:py-12 text-base sm:text-lg">No expenses recorded</p>
      ) : (
        <div className="space-y-2 sm:space-y-3">
          <h3 className="text-lg sm:text-xl font-bold text-black mb-3 sm:mb-4">Today's Expenses</h3>
          {expenses.map(expense => (
            <div key={expense.id} className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 sm:gap-4 p-3 sm:p-4 bg-gray-50 rounded-lg border border-gray-300">
              ...
              <div className="flex items-center gap-3 sm:gap-4 w-full sm:w-auto justify-between sm:justify-end">
                <span className="text-lg sm:text-xl font-bold text-orange-600">Rs.{expense.amount.toFixed(2)}</span>
                <button
